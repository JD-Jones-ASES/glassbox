// scan-text.mjs — the provider-agnostic gate. GlassBox must not name any specific course, exam, or
// standards body. This greps every committed text file for the banned terms and FAILS THE BUILD on a
// hit, so the constraint is enforced, not merely a convention. (The internal founding brief and JD.md
// are gitignored and not scanned.) This file excludes itself, since it necessarily lists the terms.
...
// "Big Idea" only as the proper-noun framework ("Big Ideas", "Big Idea 3") — case-sensitive, so it
// does not flag ordinary prose like "the big idea here". Bare "AP" is case-sensitive uppercase.
const BANNED = [
  { re: /\bAdvanced Placement\b/i, label: "Advanced Placement" },
  { re: /\bCollege Board\b/i, label: "College Board" },
  { re: /\bAP[\s-]?CSP\b/i, label: "AP CSP" },
  { re: /\bAP Computer Science\b/i, label: "AP Computer Science" },
...
if (hits.length) {
  ...
  console.error("\nGlassBox ships provider-agnostic. Remove the course/exam/standards-body reference.");
  process.exit(1);
}
